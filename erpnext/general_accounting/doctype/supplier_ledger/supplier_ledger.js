...
function load_ledger_entries(frm, silent = false) {
	...
	frappe.call({
		method: "erpnext.general_accounting.doctype.supplier_ledger.supplier_ledger.get_entries",
		args: {
		...
		callback(r) {
			...
			if (!r.message?.length && !silent) {
				frappe.msgprint(
					__(
						"No submitted supplier invoices (Entry of Entries) found for this period."
					)
				);
			}
		},
