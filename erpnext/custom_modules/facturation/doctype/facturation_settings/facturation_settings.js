frappe.ui.form.on("Facturation Settings", {
	...
	refresh_models(frm) {
		...
		frappe.dom.freeze(__("Saving settings and fetching available models..."));

		// Save first so the Password field is stored in the DB,
		// then fetch models using the server-side get_password() retrieval.
		frm.save().then(() => {
			frappe.call({
				method:
			...
			frappe.dom.unfreeze();
			frappe.msgprint(__("Please fix validation errors and save before refreshing models."));
		});
	},
});
